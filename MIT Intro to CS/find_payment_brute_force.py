...
def setBounds():
    stdin = input("Are you dealing with small loans or large (greater than one million) loans?\n" +
                  "For an explanation of the difference in how the program acts, type help. \n[default: small] ").lower()
    if stdin == "":
        stdin = 'small'
    if stdin == 'small':
        payment = 0
        ...
        print("This program works numberically, through trial and error. \n" +
              "    When the program receives the input 'small', the first payment it tries is `principal/1000' " +
              "    and increments by the same amount. \n    When it sees 'large', it tries `principle/12' " +
              "    to start and increments by `principal/12'. \nThe latter metthod is much faster, but "
              "inaccurate for small numbers.")
    else:
        print("This program only accepts 'small', 'large', and 'help' as arguments.")
        return setBounds()
